...
import mmengine
import numpy as np
from mmdet3d.datasets.convert_utils import convert_annos, get_kitti_style_2d_boxes, get_nuscenes_2d_boxes
...
def generate_nuscenes_camera_instances(info, nusc):
    ...
    for cam in camera_types:
        ...
        # list[dict]
        ann_infos = get_nuscenes_2d_boxes(nusc, cam_info["sample_data_token"], visibilities=["", "1", "2", "3", "4"])
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list["infos"])):
        ...
        for cam in ori_info_dict["cams"]:
            ...
            empty_img_info["cam2ego"] = convert_quaternion_to_matrix(
                ori_info_dict["cams"][cam]["sensor2ego_rotation"], ori_info_dict["cams"][cam]["sensor2ego_translation"]
            )
            lidar2sensor = np.eye(4)
        ...
        if "gt_boxes" in ori_info_dict:
            ...
            for i in range(num_instances):
                ...
                if ori_info_dict["gt_names"][i] in METAINFO["classes"]:
                    empty_instance["bbox_label"] = METAINFO["classes"].index(ori_info_dict["gt_names"][i])
                else:
            ...
            temp_data_info["cam_instances"] = generate_nuscenes_camera_instances(ori_info_dict, nusc)
...
def update_kitti_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info["images"]["CAM0"]["cam2img"] = ori_info_dict["calib"]["P0"].tolist()
        temp_data_info["images"]["CAM1"]["cam2img"] = ori_info_dict["calib"]["P1"].tolist()
        temp_data_info["images"]["CAM2"]["cam2img"] = ori_info_dict["calib"]["P2"].tolist()
        temp_data_info["images"]["CAM3"]["cam2img"] = ori_info_dict["calib"]["P3"].tolist()
        ...
        temp_data_info["images"]["CAM0"]["lidar2img"] = (ori_info_dict["calib"]["P0"] @ lidar2cam).tolist()
        temp_data_info["images"]["CAM1"]["lidar2img"] = (ori_info_dict["calib"]["P1"] @ lidar2cam).tolist()
        temp_data_info["images"]["CAM2"]["lidar2img"] = (ori_info_dict["calib"]["P2"] @ lidar2cam).tolist()
        temp_data_info["images"]["CAM3"]["lidar2img"] = (ori_info_dict["calib"]["P3"] @ lidar2cam).tolist()

        temp_data_info["lidar_points"]["Tr_velo_to_cam"] = Trv2c.tolist()

        # for potential usage
        temp_data_info["images"]["R0_rect"] = ori_info_dict["calib"]["R0_rect"].astype(np.float32).tolist()
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                if anns["name"][instance_id] in METAINFO["classes"]:
                    empty_instance["bbox_label"] = METAINFO["classes"].index(anns["name"][instance_id])
                else:
            ...
            if anns["gt_num"] == 0:
                ...
                for instance_id in range(num_instances):
                    ...
                    if anns["class"][instance_id] < len(METAINFO["classes"]):
                        ...
                        ignore_class_name.add(METAINFO["classes"][anns["class"][instance_id]])
...
def update_waymo_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        for cam_idx, cam_key in enumerate(camera_types):
            temp_data_info["images"][cam_key]["cam2img"] = ori_info_dict["calib"][f"P{cam_idx}"].tolist()
        ...
        for ori_sweep in ori_info_dict["sweeps"]:
            ...
            lidar_sweep["lidar_points"]["lidar_path"] = Path(ori_sweep["velodyne_path"]).name
            # image sweeps
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                dims = anns["dimensions"][instance_id]
                rots = anns["rotation_y"][:, None][instance_id]
                gt_bboxes_3d = np.concatenate([loc, dims, rots]).astype(np.float32).tolist()
                empty_instance["bbox_3d"] = gt_bboxes_3d
                empty_instance["bbox_label_3d"] = copy.deepcopy(empty_instance["bbox_label"])
                empty_instance["bbox"] = anns["bbox"][instance_id].tolist()
                empty_instance["truncated"] = int(anns["truncated"][instance_id].tolist())
                empty_instance["occluded"] = anns["occluded"][instance_id].tolist()
                empty_instance["alpha"] = anns["alpha"][instance_id].tolist()
                empty_instance["index"] = anns["index"][instance_id].tolist()
                empty_instance["group_id"] = anns["group_ids"][instance_id].tolist()
...
def generate_kitti_camera_instances(ori_info_dict):
    ...
    ann_infos = get_kitti_style_2d_boxes(ori_info_dict, occluded=[0, 1, 2, 3], annos=annos)
...
def generate_waymo_camera_instances(ori_info_dict, cam_keys):
    ...
    for cam_idx, cam_key in enumerate(cam_keys):
        ...
        ann_infos = get_kitti_style_2d_boxes(ori_info_dict, cam_idx, occluded=[0], annos=annos, dataset="waymo")
...
def parse_args():
    ...
    parser.add_argument(
        "--pkl-path", type=str, default="./data/kitti/kitti_infos_train.pkl ", help="specify the root dir of dataset"
    )
    parser.add_argument(
        "--out-dir", type=str, default="converted_annotations", required=False, help="output direction of info pkl"
    )

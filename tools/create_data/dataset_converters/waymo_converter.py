...
class Waymo2KITTI(object):
        ...
        for cam_type in self.camera_types:
            ...
            for instance in instances:
                ...
                gt_bboxes_3d = LiDARInstance3DBoxes(
                    gt_bboxes_3d[None, :]).convert_to(Box3DMode.CAM,
                                                      lidar2cam,
                                                      correct_yaw=True)
                ...
                center_2d_with_depth = points_cam2img(center_3d,
                                                      cam2img,
                                                      with_depth=True)
    ...
    def merge_trainval_infos(self):
        ...
        train_infos_path = osp.join(osp.dirname(self.save_dir),
                                    f'{self.info_prefix}_infos_train.pkl')
        val_infos_path = osp.join(osp.dirname(self.save_dir),
                                  f'{self.info_prefix}_infos_val.pkl')
        train_infos = mmengine.load(train_infos_path)
        val_infos = mmengine.load(val_infos_path)
        trainval_infos = dict(metainfo=train_infos['metainfo'],
                              data_list=train_infos['data_list'] +
                              val_infos['data_list'])
        mmengine.dump(
            trainval_infos,
            osp.join(osp.dirname(self.save_dir),

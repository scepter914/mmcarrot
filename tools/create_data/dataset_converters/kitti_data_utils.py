...
def get_label_path(
    idx, prefix, training=True, relative_path=True, exist_check=True, info_type="label_2", use_prefix_id=False
):
    return get_kitti_info_path(idx, prefix, info_type, ".txt", training, relative_path, exist_check, use_prefix_id)


def get_plane_path(
    idx, prefix, training=True, relative_path=True, exist_check=True, info_type="planes", use_prefix_id=False
):
    return get_kitti_info_path(idx, prefix, info_type, ".txt", training, relative_path, exist_check, use_prefix_id)
    ...
    def gather_single(self, idx):
        ...
        if self.label_info:
            label_path = get_label_path(
                idx, self.path, self.training, self.relative_path, info_type="label_all", use_prefix_id=True
            )
            cam_sync_label_path = get_label_path(
                idx, self.path, self.training, self.relative_path, info_type="cam_sync_label_all", use_prefix_id=True
            )
            if self.relative_path:
                label_path = str(root_path / label_path)
        ...
        while len(sweeps) < self.max_sweeps:
            ...
            prev_info["velodyne_path"] = get_velodyne_path(
                prev_idx, self.path, self.training, self.relative_path, exist_check=False, use_prefix_id=True
            )
            if_prev_exists = osp.exists(Path(self.path) / prev_info["velodyne_path"])
            if if_prev_exists:
                with open(
                    get_timestamp_path(prev_idx, self.path, self.training, relative_path=False, use_prefix_id=True)
                ) as f:
                ...
                prev_pose_path = get_pose_path(
                    prev_idx, self.path, self.training, relative_path=False, use_prefix_id=True
                )

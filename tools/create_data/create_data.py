...
from tools.create_data.dataset_converters import \
    lyft_converter as lyft_converter
from tools.create_data.dataset_converters import \
    nuscenes_converter as nuscenes_converter
from tools.create_data.dataset_converters import semantickitti_converter
from tools.create_data.dataset_converters.create_gt_database import (
    GTDatabaseCreater, create_groundtruth_database)
from tools.create_data.dataset_converters.update_infos_to_v2 import \
    update_pkl_infos
...
def lyft_data_prep(root_path, info_prefix, version, max_sweeps=10):
    ...
    Args:
        ...
        max_sweeps (int, optional): Number of input consecutive frames.
            Defaults to 10.
    """
    lyft_converter.create_lyft_infos(root_path,
                                     info_prefix,
                                     version=version,
                                     max_sweeps=max_sweeps)
...
def scannet_data_prep(root_path, info_prefix, out_dir, workers):
    ...
    """
    indoor.create_indoor_info_file(root_path,
                                   info_prefix,
                                   out_dir,
                                   workers=workers)
    info_train_path = osp.join(out_dir, f'{info_prefix}_infos_train.pkl')
...
def waymo_data_prep(root_path,
    ...
    if not only_gt_database:
        ...
        from tools.create_data.dataset_converters.waymo_converter import \
            create_ImageSets_img_ids
        create_ImageSets_img_ids(out_dir, splits)

    GTDatabaseCreater('WaymoDataset',
                      out_dir,
                      info_prefix,
                      f'{info_prefix}_infos_train.pkl',
                      relative_path=False,
...
parser.add_argument('--with-plane',
                    action='store_true',
...
if __name__ == '__main__':
    ...
    if args.dataset == 'kitti':
        ...
        lyft_data_prep(root_path=args.root_path,
                       info_prefix=args.extra_tag,
                       version=train_version,
                       max_sweeps=args.max_sweeps)
        test_version = f'{args.version}-test'
        ...
        scannet_data_prep(root_path=args.root_path,
                          info_prefix=args.extra_tag,
                          out_dir=args.out_dir,
                          workers=args.workers)
    elif args.dataset == 's3dis':
        s3dis_data_prep(root_path=args.root_path,
                        info_prefix=args.extra_tag,
                        out_dir=args.out_dir,
                        workers=args.workers)
    elif args.dataset == 'sunrgbd':
        sunrgbd_data_prep(root_path=args.root_path,
                          info_prefix=args.extra_tag,
                          out_dir=args.out_dir,
                          workers=args.workers)
    elif args.dataset == 'semantickitti':
        semantickitti_data_prep(info_prefix=args.extra_tag,
                                out_dir=args.out_dir)
    else:

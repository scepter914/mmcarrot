...
from tools.create_data.dataset_converters import indoor_converter as indoor
from tools.create_data.dataset_converters import kitti_converter as kitti
from tools.create_data.dataset_converters import lyft_converter as lyft_converter
from tools.create_data.dataset_converters import nuscenes_converter as nuscenes_converter
from tools.create_data.dataset_converters import semantickitti_converter
from tools.create_data.dataset_converters.create_gt_database import GTDatabaseCreater, create_groundtruth_database
from tools.create_data.dataset_converters.update_infos_to_v2 import update_pkl_infos
...
def waymo_data_prep(
    ...
    """
    from tools.create_data.dataset_converters import waymo_converter as waymo
    ...
    if not only_gt_database:
        ...
        from tools.create_data.dataset_converters.waymo_converter import create_ImageSets_img_ids
...
if __name__ == "__main__":
    ...
    if args.dataset == "kitti":
        ...
        if args.only_gt_database:
            create_groundtruth_database(
                "NuScenesDataset", args.root_path, args.extra_tag, f"{args.extra_tag}_infos_train.pkl"
            )
        else:
        ...
        lyft_data_prep(
            root_path=args.root_path, info_prefix=args.extra_tag, version=train_version, max_sweeps=args.max_sweeps
        )
        test_version = f"{args.version}-test"
        lyft_data_prep(
            root_path=args.root_path, info_prefix=args.extra_tag, version=test_version, max_sweeps=args.max_sweeps
        )
    elif args.dataset == "scannet":
        scannet_data_prep(
            root_path=args.root_path, info_prefix=args.extra_tag, out_dir=args.out_dir, workers=args.workers
        )
